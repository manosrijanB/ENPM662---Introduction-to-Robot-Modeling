...
# Potential Energy
P = m[0]*g*0.05 + sum([m[i]*T[i][2, 3]*g for i in range(7) ])
# Torque due to gravity
g_torque = [diff(P, thetas[i]) for i in range(7)]
g_torque = lambdify(thetas, g_torque, 'numpy')
...
for _ in range(num_points):
    ...
    # Forward Kinematics and Torque
    Xp_t = Fwd_Kin(*q_curr)
    joint_torque_t = g_torque(*q_curr) - (J_sub.T @ F_ext)
    # add to list
    Xp.append(Xp_t)
    joint_torques.append(joint_torque_t)
    # update joint angles
...
for i in range(7):
    plt.plot(joint_torques[:, i], 'b')
    plt.title(f"Joint {i}")
    plt.xlabel("time")
    plt.ylabel("joint torque")
    plt.savefig(f"HW5_{i}.png")
    plt.show()